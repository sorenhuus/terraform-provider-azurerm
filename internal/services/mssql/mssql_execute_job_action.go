	...
	response.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"wait_for_completion": schema.BoolAttribute{
				...
				MarkdownDescription: "Whether to poll the job execution for completion and report its status. Defaults to `false`",
			},
			...
			"timeout": schema.StringAttribute{
				...
				MarkdownDescription: "Timeout duration for the job execution. Defaults to `15m`.",
			},
		},
	}
}
...
func (m *MsSqlExecuteJobAction) Invoke(ctx context.Context, request action.InvokeRequest, response *action.InvokeResponse) {
	...
	if model.WaitForCompletion.ValueBool() {
		...
		if err := poller.PollUntilDone(context.WithValue(ctx, "response", response)); err != nil {
			sdk.SetResponseErrorDiagnostic(response, "waiting for completion", err)
			return
		}
